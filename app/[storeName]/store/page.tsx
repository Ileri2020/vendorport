    ...
    >
      <div className="w-full md:container md:mx-auto max-w-7xl">
        <div className="flex flex-col md:flex-row justify-between items-stretch bg-accent/10 p-2 md:p-3 rounded-2xl mb-3 gap-3">
          {/* Chat / Contact card — always visible */}
          <div className="flex-1 flex items-center gap-3 md:gap-4 bg-background/50 p-3 rounded-xl border border-accent/20">
            ...
            <div>
              ...
              <p className="text-[10px] md:text-xs text-muted-foreground font-bold">
                {isPharmacy ? "Our pharmacists are ready to consult." : "Our team is happy to assist you."}
              </p>
            </div>
            <Link href="/contact" className="ml-auto">
              <Button size="sm" variant="default" className="bg-accent hover:bg-accent/90 h-8 text-[10px] font-black uppercase">
                {isPharmacy ? "Talk Now" : "Chat Us"}
              </Button>
            </Link>
          </div>

          {/* Scarce Medicine card — pharmacy only */}
          {isPharmacy && (
