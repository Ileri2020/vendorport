...
export default function AnalyticsDashboard() {
  ...
  useEffect(() => {
    if (!queryParams.from || !queryParams.to || !queryParams.businessId) return;

    setIsLoading(true);
    fetch(
      `/api/analytics?from=${queryParams.from}&to=${queryParams.to}&businessId=${queryParams.businessId}`
    )
      .then((res) => res.json())
      .then((json) => {
        setData(json);
  ...
  }, [queryParams.from, queryParams.to, queryParams.businessId]);

  if (status === "loading" || !isBusinessOwner || !currentBusiness) {
    return null;
  }
